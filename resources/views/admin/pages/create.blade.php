...
@section('content')
    <div class="mb-10">
        ...
        <h1 class="text-4xl font-outfit font-extrabold text-slate-900 tracking-tight">Yeni Sayfa Oluştur</h1>
    </div>

    <form action="{{ route('admin.pages.store') }}" method="POST" enctype="multipart/form-data" class="grid grid-cols-1 lg:grid-cols-3 gap-10">
        @csrf
        ...
        <div class="lg:col-span-2 space-y-8">
            <div class="bg-white p-8 rounded-[32px] shadow-sm border border-slate-200/60">
                <div class="mb-8">
                    ...
                    <input type="text" name="title" value="{{ old('title') }}" class="w-full bg-slate-50 border-transparent rounded-2xl focus:bg-white focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 transition-all p-4 text-lg font-medium border" required placeholder="Örn: Hakkımda">
                </div>
                
                <div class="editor-wrapper">
                    <label class="block text-sm font-bold text-slate-700 mb-3 ml-1">İçerik (HTML desteklenir)</label>
                    <textarea name="content" id="editor" rows="20" class="w-full bg-slate-50 border-transparent rounded-2xl focus:bg-white focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 transition-all p-4 border">{{ old('content') }}</textarea>
                </div>
            </div>
        </div>
        ...
        <div class="space-y-8">
            ...
            <div class="bg-white p-8 rounded-[32px] shadow-sm border border-slate-200/60 overflow-hidden relative">
                <div class="absolute top-0 left-0 w-full h-1.5 bg-gradient-to-right from-emerald-500 to-teal-500"></div>
                <h3 class="font-outfit font-bold text-slate-800 mb-6 flex items-center">
                    <i class="fas fa-plus-circle mr-3 text-emerald-500"></i> Yayınla
                </h3>
